...
int log_idx = 0;
bool __bdebug_kernel_started = FALSE;

void DebugKernelStarted(void) { __bdebug_kernel_started = TRUE; }
...
void KLog(const char *format, ...) {
  ...
  if (__bdebug_enabled) {
    if (__bdebug_kernel_started) {
//      Vsnprintf(&klog_entry[0][0], KLOG_WIDTH, format, ap);
//      Debug(&klog_entry[0][0]);
    } else {
...
/*
    Scrolls the screen by a character height.  A bit slow as reading from screen
   buffer
...
static void PrintChar(int x, int y, char ch) {
  ...
  for (r = 0; r < 8; r++) {
    for (c = 0; c < 8; c++) {
      ...
      if ((1 << c) & bit_row) {
        *(uint16 *)((vm_addr)screen_buf + (y + r) * screen_pitch +
                    ((x + c) * 2)) = pen_color;
      }
    }
  }
